...
import { ShortUserDto, UiCustomizationSettingsDto } from '@shared/service-proxies/service-proxies';
...
export abstract class AppComponentBase {
    ...
    getUserProfilePitureUrl(userId: number, token: string) {
        return AppConsts.remoteServiceBaseUrl
            + '/Profile/GetProfilePictureByUser?userId=' + userId
            + '&' + AppConsts.authorization.encrptedAuthTokenName + '=' + encodeURIComponent(token);
    }

    getFullName(userInfo: { name: string, surname: string }) {
        return `${userInfo.name} ${userInfo.surname}`;
    }

    protected addSubscriptionToClear(sub: Subscription) {
        this.subscriptionsToClear.push(sub);
    }

    protected clearSubscriptions() {
        this.subscriptionsToClear.forEach(sub => sub.unsubscribe());
        this.subscriptionsToClear = [];
    }
