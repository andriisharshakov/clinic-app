...
export class ChatBarComponent extends AppComponentBase implements OnInit, AfterViewInit {
    ...
    get selectedUser(): ChatFriendDto {
        return this._selectedUser || {} as ChatFriendDto;
    }
    ...
    ngAfterViewInit(): void {
        ...
        // Had to use the p-fileUpload's inner input element title because this import overlays the p-fileUpload's
        // container with z-index. If not that z-index, we'd simply put the title in the `span[class="fileinput-button"]`
        // elements directly in the HTML template.
        this.chatImageUpload.advancedFileInput.nativeElement.setAttribute('title', this.l('Upload image'));
        this.chatFileUpload.advancedFileInput.nativeElement.setAttribute('title', this.l('Upload file'));
    }
    ...
    registerEvents(): void {
        ...
        function onchatbarToggled(userIds) {
            const user = self.getFriendOrNull(userIds.targetUserId, userIds.targetTenantId);
            if (!user) {
                return;
            }

            self.isOpen = true;
            self.mQuickSidebarOffcanvas.show();
            self.selectFriend(user);
            self.pinned = true;
            self.scrollToBottom();
        }

        abp.event.on('app.chat.chatbarToggled', user => {
            self._zone.run(() => {
                onchatbarToggled(user);
            });
        });
